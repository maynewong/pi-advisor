...
describe("resuming a completed run", () => {
	...
	test("delivers resume events to live subscribers without reopening wait() or events", async () => {
		const manager = new SubagentManager({ cwd: "/repo", createDriver: resumableFactory() });
		const handle = manager.spawn(PROFILE, "task");
		const live: string[] = [];
		const throwing = vi.fn(() => { throw new Error("observer boom"); });
		const error = vi.spyOn(console, "error").mockImplementation(() => {});
		handle.subscribe(throwing);
		handle.subscribe((event) => live.push(event.type));
		const iterator: string[] = [];
		const consume = (async () => {
			for await (const event of handle.events) iterator.push(event.type);
		})();
		...
		expect(iterator.at(-1)).toBe("completed");

		const later: string[] = [];
		handle.subscribe((event) => later.push(event.type));
		const second = await handle.resume("dig deeper");
		expect(second.text).toBe("follow-up: dig deeper");
		expect(await handle.wait()).toBe(first);
		expect(later).toContain("file_read");
		expect(later).toContain("completed");
		expect(live.filter((type) => type === "file_read")).toHaveLength(2);
		expect(iterator.filter((type) => type === "file_read")).toHaveLength(1);
		expect(throwing.mock.calls.length).toBeGreaterThan(1);
		expect(error).toHaveBeenCalled();
		error.mockRestore();

		const leftover: string[] = [];
		for await (const event of handle.events) leftover.push(event.type);
		expect(leftover).toEqual([]);
	});

	test("rejects a concurrent resume of the same completed run", async () => {
		const gate = deferred<{ text: string; usage: ReturnType<typeof usage> }>();
		const factory: RuntimeDriverFactory = async (_request, emit) => ({
			async run() { return { text: "first", usage: usage(1) }; },
			async resume(message: string) {
				emit({ type: "progress", text: message });
				return gate.promise;
			},
			async abort() {},
		});
		const handle = new SubagentManager({ cwd: "/repo", createDriver: factory }).spawn(PROFILE, "task");
		await handle.wait();
		const first = handle.resume("one");
		await vi.waitFor(() => expect(handle.status).toBe("running"));
		await expect(handle.resume("two")).rejects.toThrow(/already active|no longer completed/);
		gate.resolve({ text: "follow-up: one", usage: usage(2) });
		await expect(first).resolves.toMatchObject({ text: "follow-up: one" });
		await expect(handle.resume("three")).resolves.toMatchObject({ status: "completed" });
	});
