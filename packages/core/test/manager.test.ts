...
describe("SubagentManager", () => {
	...
	test("uses the profile context budget when constructing a selected context packet", async () => {
		const cwd = await mkdtemp(join(tmpdir(), "subagent-context-budget-"));
		...
		const factory: RuntimeDriverFactory = async (request) => ({
			async run() {
				expect(request.prompt).toContain("## File scope: a.txt");
				expect(request.prompt).not.toContain("## File scope: b.txt");
				expect(request.prompt).not.toContain("a".repeat(80));
				return { text: "ok" };
			},
			async abort() {},
		});

		const result = await new SubagentManager({ cwd, createDriver: factory }).spawn(
			{ ...PROFILE, contextMaxBytes: 50 },
			"inspect",
			{ context: { files: ["a.txt", "b.txt"] } },
		).wait();

		expect(result.disclosure.contextSources).toEqual(["file:a.txt"]);
		expect(result.disclosure.truncated).toEqual(["file:a.txt", "file:b.txt"]);
	});
	...
	test("passes task and spawn metadata to concurrency-key resolution", async () => {
		const resolver = vi.fn(() => "external:target");
		const manager = new SubagentManager({
			cwd: "/repo",
			resolveConcurrencyKey: resolver,
			createDriver: async () => ({ run: async () => ({ text: "ok" }), abort: async () => {} }),
		});
		await manager.spawn(PROFILE, "provider task", { metadata: { runtime: "external" } }).wait();
		expect(resolver).toHaveBeenCalledWith(PROFILE, "provider task", expect.objectContaining({ metadata: { runtime: "external" } }));
	});
	...
	test("maps a driver-reported timeout to the timeout terminal status", async () => {
		const factory: RuntimeDriverFactory = async () => ({
			run: async () => ({ text: "partial", error: { kind: "timeout", message: "remote wait expired" } }),
			async abort() {},
		});
		const result = await new SubagentManager({ cwd: "/repo", createDriver: factory }).spawn(PROFILE, "remote timeout").wait();
		expect(result).toMatchObject({ status: "timeout", text: "partial", error: { kind: "timeout" } });
	});
	...
	test("fails fast when an advisor override adds a dangerous tool", () => {
		const manager = new SubagentManager({ cwd: "/repo", createDriver: async () => ({ run: async () => ({ text: "ok" }), async abort() {} }) });
		const advisor = { ...PROFILE, name: "advisor" };

		expect(() => manager.spawn(advisor, "review", { overrides: { tools: ["read", "write"] } })).toThrow(/read-only.*write/i);
		expect(() => manager.spawn(advisor, "review", { overrides: { name: "worker", tools: ["read", "bash"] } })).toThrow(/read-only.*bash/i);
		expect(() => manager.spawn(advisor, "review", { overrides: { tools: ["read", "edit"] } })).toThrow(/read-only.*edit/i);
	});
