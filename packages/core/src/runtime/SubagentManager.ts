...
import { assertAdvisorReadOnly } from "./piSdkDriverSupport.ts";
...
import { RuntimeProviderError } from "./provider.ts";
...
export interface SubagentManagerOptions {
	...
	resolveConcurrencyKey?: (profile: SubagentProfile, task: string, options: SpawnOptions) => string | undefined;
	maxDepth?: number;
	artifactsDir?: string;
...
export class SubagentManager {
	...
	spawn(profile: SubagentProfile, task: string, options: SpawnOptions = {}): SubagentHandle {
		...
		const advisorName = profile.name === "advisor" ? profile.name : undefined;
		assertAdvisorReadOnly(advisorName ? { ...effective, name: advisorName } : effective);
		const handle = new ManagedSubagentHandle(randomUUID(), effective);
		const concurrencyKey = options.metadata?.concurrencyKey as string | undefined ?? this.options.resolveConcurrencyKey?.(effective, task, options);
	 ...
	 * resumable (completed runs whose driver supports resume). Driver failures surface as result data;
	 * invalid requests reject the returned promise.
	 */
	async resume(id: string, message: string): Promise<SubagentResult> {
		...
		if (handle.status !== "completed") throw new Error(`Subagent ${id} is already active or no longer completed`);
		...
		const result = outcome.error
			? this.baseResult(run, this.statusForDriverError(outcome.error.kind), outcome.text, outcome.error, disclosure, outcome.usage, undefined, outcome.sessionFile, undefined, outcome.messages, outcome.items)
			: (() => {
				const resolved = resolveOutput(run.profile.output, outcome.text, outcome.submitted);
				return this.baseResult(run, resolved.error ? "failed" : "completed", outcome.text, resolved.error, disclosure, outcome.usage, resolved.output, outcome.sessionFile, outcome.stoppedBy, outcome.messages, outcome.items);
			})();
	...
	private async execute(run: QueuedRun): Promise<void> {
		...
		try {
			...
			const packet = await buildContextPacket(options.context, {
				cwd: runCwd,
				...(profile.contextMaxBytes !== undefined ? { maxTotalBytes: profile.contextMaxBytes } : {}),
			});
			...
			if (outcome.error) {
				const status = this.statusForDriverError(outcome.error.kind);
				const result = this.baseResult(run, status, outcome.text, outcome.error, disclosure, outcome.usage, undefined, outcome.sessionFile, undefined, outcome.messages, outcome.items);
			...
			const kind = aborted ? "aborted" : error instanceof RuntimeProviderError ? error.kind : "model";
			const result = this.baseResult(run, aborted ? "aborted" : this.statusForDriverError(kind), "", { message: aborted ? "Subagent aborted" : message, kind });
	...
	private statusForDriverError(kind: NonNullable<SubagentResult["error"]>["kind"]): SubagentResult["status"] {
	...
	private baseResult(
		...
		items?: SubagentResult["items"],
	): SubagentResult {
		return { status, text, usage: { ...usage }, disclosure, ...(error ? { error } : {}), ...(stoppedBy ? { stoppedBy } : {}), ...(output !== undefined ? { output } : {}), ...(sessionFile ? { sessionRef: { file: sessionFile } } : {}), ...(messages ? { messages } : {}), ...(items ? { items } : {}) };
	}
